...
class _TrashScreenState extends State<trashscreen> {
  ...
  Future<void> _loadData() async {
    ...
    final trashCategory = categories.firstWhere(
          (cat) => cat['name'] == '분리수거',
      orElse: () => categories.isNotEmpty ? categories.first : {},
    );

    if (trashCategory.isNotEmpty) {
      trashCategoryId = trashCategory['_id'];
    }
  }

  Future<void> _addDuty() async {
    if (selectedPersonId == null || trashCategoryId == null) {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('분리수거 일정이 등록되었습니다.')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    return Scaffold(
      ...
      body: Consumer<AppState>(
        builder: (context, appState, child) {
          // 분리수거 카테고리 일정만 필터링
          final trashSchedules = appState.choreSchedules
              .where((schedule) =>
          schedule['category'] != null &&
              schedule['category']['name'] == '분리수거')
              .toList();
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    itemCount: trashSchedules.length,
                    itemBuilder: (context, index) {
                      final schedule = trashSchedules[index];
